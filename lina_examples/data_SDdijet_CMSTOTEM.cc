...
void data_SDdijet_CMSTOTEM(string const& outputFileName = "data_SDdijet_CMSTOTEM.root", const Int_t nevt_max = -1){
  ...
  histosTH1F["xi_cms_totem_background_simulated_right"] = new TH1F("xitotem_xicms_rightRPs_simulated_right", "Right RPs" , 20 , -0.4 , 0.4);
  histosTH1F["xi_cms_totem_background_simulated_left"] = new TH1F("xitotem_xicms_rightRPs_simulated_left", "Left RPs" , 20 , -0.4 , 0.4);
...
//     tree->SetBranchAddress("particleFlow",&pFlow_coll);
    ...
    for(int i_evt = 0; i_evt < nev && i_tot < nevt_max_corr; ++i_evt , ++i_tot){
	 ...
	 if (eta < eta_min) {eta_min = eta; PF_eta_min = true;}
      ...
      bool rp_track_accept = ( rp_track_valid_120 && rp_track_valid_124 && cut_rp_124 )  || ( rp_track_valid_121 && rp_track_valid_125 && cut_rp_125 );// || ( track_valid_122 && track_valid_123 );
      ...
      if(good_proton_right && rp_track_accept){ 
         ...
         if (t_region_right) histosTH1F["proton_right_xi_kint"]->Fill(-xi_proton_right, event_weight );
         ...
         if (xi_region_right && t_region_right) {
             ...
             if(xi_minus_Reco+xi_proton_right>0){
               ...
               histosTH1F["proton_right_t_halo_constbin"]->Fill(fabs(t_proton_right), event_weight );
             }
             ...
             if (xi_minus_Reco+xi_proton_right>0) continue;
                ...
                histosTH1F["proton_right_t_signal_constbin"]->Fill( fabs(t_proton_right), event_weight );
                histosTH1F["proton_right_t_signal_eff"]->Fill( fabs(t_proton_right), eff );
                histosTH1F["proton_right_t_signal_effweight"]->Fill( fabs(t_proton_right), event_weight_eff );
                histosTH1F["proton_right_t_signal_averagept_eff"]->Fill( fabs(t_proton_right), averagept_eff );
                histosTH1F["proton_right_t_signal_averagept_effweight"]->Fill( fabs(t_proton_right), event_weight_averagept_eff );
             ...
             if( pfJet_coll->size() > 0 ){
               MyBaseJet const& leadingJet = ( pfJet_coll->at(0) ).mapjet[ jetCorrName ];
               histosTH2F["proton_right_t_vs_leadingJet_pt"]->Fill( leadingJet.Pt(), fabs(t_proton_right), event_weight );
             }
         }
      }
...
*/
  ...
  //beam halo
  int halo_signal_pt30_right = 0;
  int halo_signal_pt30_left = 0;
  for (int a = 1; a<=histosTH1F["proton_right_xi_cut"]->GetEntries(); a++){
       ...
       histosTH1F["xi_cms_totem_background_simulated_right"]->Fill(xi_cms_right-xi_totem_right,1.0); 
       if (xi_cms_right-xi_totem_right<0.){++halo_signal_pt30_right;}
   }   
  cout<<"background_right:  "<<halo_signal_pt30_right<<endl;

  for (int b = 1; b<=histosTH1F["proton_left_xi_cut"]->GetEntries(); b++){
       double xi_cms_left = histosTH1F["xi_plus_Reco"]->GetRandom();
       double xi_totem_left = histosTH1F["proton_left_xi_cut"]->GetRandom();
       histosTH1F["xi_cms_totem_background_simulated_left"]->Fill(xi_cms_left-xi_totem_left,1.0); 
       if (xi_cms_left-xi_totem_left<0.){++halo_signal_pt30_left;}
   }   
   cout<<"background_left:  "<<halo_signal_pt30_left<<endl;

  
   double nevents_t_halo_pt30_right[11];
   double nevents_t_halo_pt30_left[11];
   double halo_nosignal_pt30_right = histosTH1F["proton_right_t_halo"]->GetEntries();
   double halo_nosignal_pt30_left = histosTH1F["proton_left_t_halo"]->GetEntries();
   double scale_pt30_right = halo_signal_pt30_right/halo_nosignal_pt30_right;
   double scale_pt30_left = halo_signal_pt30_left/halo_nosignal_pt30_left;
   double nevents_data_right[11];
   double nevents_data_left[11];
   double nevents_true_right[11];
   double nevents_true_left[11];
   ...
   for (int c = 1; c<=11; c++){
       nevents_t_halo_pt30_right[c] = histosTH1F["proton_right_t_halo"]->GetBinContent(c)*scale_pt30_right;
       nevents_t_halo_pt30_left[c] = histosTH1F["proton_left_t_halo"]->GetBinContent(c)*scale_pt30_left;
       nevents_data_right[c] = histosTH1F["proton_right_t_signal"]->GetBinContent(c);
       nevents_data_left[c] = histosTH1F["proton_right_t_signal"]->GetBinContent(c);
       nevents_true_right[c] = nevents_data_right[c]-nevents_t_halo_pt30_right[c]; 
       nevents_true_left[c] = nevents_data_left[c]-nevents_t_halo_pt30_left[c]; 
       histosTH1F["halo_right_pt30"] -> SetBinContent(c, nevents_t_halo_pt30_right[c]);
       histosTH1F["halo_right_pt30"] -> SetBinContent(c, nevents_t_halo_pt30_left[c]);
       histosTH1F["proton_right_t_true"] -> SetBinContent(c, nevents_true_right[c]);
       histosTH1F["proton_right_t_true"] -> SetBinContent(c, nevents_true_left[c]);

       cout << c <<"  data_right: "<<nevents_data_right[c]<<"  halo_right: "<<nevents_t_halo_pt30_right[c]<<"   true_right: "<<nevents_true_right[c] << endl
                 <<"  data_left:  "<<nevents_data_left[c]<<"  halo_left: "<<nevents_t_halo_pt30_left[c]<<"   true_left: "<<nevents_true_left[c] << endl;
   }

   for (int d = 1; d<=nbins_const; d++){
       nevents_t_halo_pt30_constbin[d] = histosTH1F["proton_right_t_halo_constbin"]->GetBinContent(d)*scale_pt30_right;
       nevents_data_constbin[d] = histosTH1F["proton_right_t_signal_constbin"]->GetBinContent(d);
       nevents_true_constbin[d] = nevents_data_constbin[d]-nevents_t_halo_pt30_constbin[d]; 
       histosTH1F["halo_right_pt30_constbin"] -> SetBinContent(d, nevents_t_halo_pt30_constbin[d]);
       histosTH1F["proton_right_t_true_constbin"] -> SetBinContent(d, nevents_true_constbin[d]);
   }
